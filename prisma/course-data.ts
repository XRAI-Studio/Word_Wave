// LingoDuo Spanish course content — 9 sections, in the spirit of Duolingo's
// Spanish course arc (basics → daily life → travel → tenses → conversation →
// culture → mastery → verb conjugation).
//
...
//
// Sentence rules:
//  - `es` must be exactly its bank tokens joined by single spaces
...
const w = (term: string, translation: string): WordDef => ({ term, translation });
const s = (es: string, en: string, bank: string[], words: string[]): SentenceDef => ({ es, en, bank, words });
